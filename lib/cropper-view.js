...
var View = BaseView.extend(bindTransforms).extend({
    ...
    events: {
        'mousedown [role=cropper-image-scale]' : 'onZoomStart',
        'mousemove [role=cropper-image-scale]' : 'onZoom',
        'change    [role=cropper-image-scale]' : 'updateScaleFromSlider',
    ...
    bindings: {
        'model.src': {
            type: 'attribute',
            name: 'src',
            selector: '[role=cropper-mask] img'
        },
        'model.minScale': {
            type: 'attribute',
            name: 'min',
            selector: '[role=cropper-image-scale]'
        },
        'model.maxScale': {
            type: 'attribute',
            name: 'max',
    ...
    render: function () {
        ...
        var maskEl = this.getByRole('cropper-mask');
        ...
        var imgEl = this.get('[role=cropper-mask] img');
